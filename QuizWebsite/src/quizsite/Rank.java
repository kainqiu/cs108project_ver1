...
public class Rank {
	...
	static public ArrayList<QuizInfo> getQuizzesByTag(DBConnection dbCon, String tag) {
		ArrayList<QuizInfo> sameTag = new ArrayList<QuizInfo>();
		try {
			String selectSQL = "SELECT quizId, title, tag FROM tags";
			PreparedStatement preStmt = dbCon.getConnection().prepareStatement(selectSQL);
			ResultSet rs = preStmt.executeQuery();
			while(rs.next()) {
				if(rs.getString("tag").equals(tag.toLowerCase())){
					QuizInfo qi = new QuizInfo(rs.getInt("quizId"), rs.getString("title"));
					sameTag.add(qi);
				}
			}
		} catch (SQLException e) {
